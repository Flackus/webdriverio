...
let WebdriverIO = function (args, modifier) {
    ...
    let stackDepth = 10
    if (process.env.WEBDRIVERIO_STACK_LIMIT) {
        stackDepth = parseInt(process.env.WEBDRIVERIO_STACK_LIMIT, 10)
    }
    ...
    function fail (e, onRejected) {
        ...
        let customStack = commandList
            .map(command => `    at ${command.stack}`)
            .reverse()
            .slice(0, stackDepth)
            .join('\n')
    ...
    function unit (lastPromise) {
        ...
        client.next = function (func, args, name) {
            ...
            return this.lastPromise.then((val) => {
                ...
                commandList.push({name, args, timestamp, stack: client.stack})
    ...
    unit.lift = function (name, func) {
        ...
        commandGroup[name] = function (...args) {
            ...
            client.stack = `${name}(${sanitize.args(args)}) (${fileAndPosition})`
